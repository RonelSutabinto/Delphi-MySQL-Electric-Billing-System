...
type
  TAgingAnalysisForm = class(TForm)
    Panel1: TPanel;
    Label14: TLabel;
    Label27: TLabel;
    Label29: TLabel;
...
var
  AgingAnalysisForm: TAgingAnalysisForm;
  Lst              : Text;
  HeadingSW        : Boolean;
  PageCount        : Word;
  LineCount        : Word;
...
procedure TAgingAnalysisForm.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
   Action := caFree;
   AgingAnalysisForm := Nil;
end;

procedure TAgingAnalysisForm.PDJSillyLabel11Click(Sender: TObject);
begin
  Close;
end;

Procedure TAgingAnalysisForm.CloseTable(Table: TTable);
begin
  if (Table.Active) then
    Table.Close;
end;

procedure TAgingAnalysisForm.PDJSillyLabel12Click(Sender: TObject);
var
  Months           : Integer;
...
begin
  ...
  Writeln(lst,#27 + '4'+'AGING ANALYSIS REPORT'+#27+'5');
  Writeln(lst,'REFERENCE BILL MONTH     : '+AgingBillMonth.Text);
  Writeln(lst,'AREA : '+AgingArea.Text, '   BOOK : '+ AgingBook.Text, '   ADDRESS : '+VicinityAddress.Text);
  Write(Lst,#27+'M');
  Writeln(lst,'|------------------------------------------------------------------------------------------------------------------------------------|');
...
var
  locateval: Array[0..1] of Variant;
begin

  locateval[0] :=  AgingArea.Text;
  locateval[1] :=  AgingBook.Text;

  Vicinity.Open;
  if not (Vicinity.Locate('Area;Book', VarArrayOf(locateval), [loCaseInsensitive])) then
  begin
    ShowMessage('Area / Book invalid.');
    AgingBook.Text := '';
    AgingArea.SetFocus;
    Exit;
  end;
  ...
  Master.Filter :=  'Area = ' + QuotedStr(AgingArea.Text) +
                  ' and Book = ' + QuotedStr(AgingBook.Text);
  ...
  try
    ...
    begin
      ...
      if (AgeTotal > 0) then
      begin
        Writeln(lst,'|',JustStr(MasterAccountNumber.Text,' ',8,0)+'  '+MasterRateCode.Text+'   ',
      ...
      If (LineCount = 50) and not Master.eof then
      begin
        LineCount := 0;
        Inc(PageCount);
        Write(Lst,#12);
