...
type
  TBillMasterMainRecordForm = class(TForm)
    ...
    cipherRangeQcbook: TStringField;
    CB_isFixedDemand: TNxCheckBox;
...
begin
  ...
  If AccountCode = 0 then
    ...
    begin
      ...
      if CB_isFixedDemand.Checked then
          Updatemaster.ParamByName('isFixDemand').AsInteger  := 1
      else
          Updatemaster.ParamByName('isFixDemand').AsInteger  := 0;
...
procedure TBillMasterMainRecordForm.CRDBGrid4CellClick(Column: TColumn);
var
  cbookTmp : String;
begin
  cbookTmp    := cipherRangeQcbook.Text;
  CBook.Text  := cbookTmp;

  CipherBookMaster.close;
  CipherBookMaster.ParamByName('cbook').Text := cbookTmp;
